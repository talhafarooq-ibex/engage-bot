...
from routers.dashboard.utilities.filters import \
    filter_records_analytics as filter_records
...
async def total_bots_agent(request: Request):
    try:
        ...
        slug = bots_record['bot_name'] + slug_db
        db_temp = await connect(slug)
        messages_collections = db_temp['messages']
        ...
        for record in workspaces_records:
            ...
            for message in message_records:
                ...
                if not message['transfer_conversation'] and (not message['end_conversation'] and expiration_time > latest_timestamp):
                    available_agents -= 1
